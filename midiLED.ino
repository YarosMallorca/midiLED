...
Adafruit_NeoPixel LED = Adafruit_NeoPixel(_numLED, pinLED, NEO_GRB + NEO_KHZ800);

void noteOn(byte ch, byte p, byte v) {
  LED.setPixelColor((p + (60 * (ch - 1))) - 36, _r[v], _g[v], _b[v]);
}

void noteOff(byte ch, byte p, byte v) {
  LED.setPixelColor((p + (60 * (ch - 1))) - 36, 0, 0, 0);
}
