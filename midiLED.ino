...
// #     GENERAL CONFIG      #
// # ----------------------- #
#define _numLED 128   // Number of LEDs in each of the LED Strips.
                      // If the number of LEDs is different in each strip, set the maximum number of LEDs here.
#define pStart 36     // First note in array (36 is default, maps to C1 in Ableton)
#define bLED 255      // Maximum Overall Brightness of LEDs (0-255) [Set to 255 if you have a power supply]
#define rDelay 80     // Refresh Delay (lower = more FPS) if set too low will lag on high LED Counts
#define ledsPerNote 8 // Number of Leds controlled per one MIDI Note
...
// #     LED STRIP CONFIG      #
// # ------------------------- #
#define numPins 4 // Number of LED Strips
// ! WARNING ! This should match with the length of the _LED array
Adafruit_NeoPixel _LED[numPins] = {
    Adafruit_NeoPixel(_numLED, 3, NEO_GRB + NEO_KHZ800),
    Adafruit_NeoPixel(_numLED, 4, NEO_GRB + NEO_KHZ800),
    Adafruit_NeoPixel(_numLED, 5, NEO_GRB + NEO_KHZ800),
    Adafruit_NeoPixel(_numLED, 6, NEO_GRB + NEO_KHZ800)};
...
void noteOn(byte ch, byte p, byte v)
{
  ...
  {
    for (int i = 0; i < ledsPerNote; i++)
    {
      int ledIndex = ((p - pStart) * ledsPerNote + i) + (ledsPerNote * (ch - 1));
      _LED[pin].setPixelColor(ledIndex, _r[v], _g[v], _b[v]);
    }
    count = 0;
  }
}
...
void noteOff(byte ch, byte p, byte v)
{
  ...
  if (pin >= 0 && pin < numPins)
  {
    ...
    {
      int ledIndex = ((p - pStart) * ledsPerNote + i) + (ledsPerNote * (ch - 1));
      _LED[pin].setPixelColor(ledIndex, 0, 0, 0);
    }
    count = 0;
  }
}
