...
#define notesPerChannel 127 // Number of notes active in one MIDI Channel
#define ledsPerNote  1  // Number of Leds controlled per one MIDI Note (Change for multiple LEDs per note)
...
void noteOn(byte ch, byte p, byte v) {
  for (int i = 0; i < ledsPerNote; i++) {
    int ledIndex = ((p - pStart) * ledsPerNote + i) + (notesPerChannel * ledsPerNote * (ch - 1));
    _LED.setPixelColor(ledIndex, _r[v], _g[v], _b[v]);
  }
  count = 0;
}
...
void noteOff(byte ch, byte p, byte v) {
  for (int i = 0; i < ledsPerNote; i++) {
    int ledIndex = ((p - pStart) * ledsPerNote + i) + (notesPerChannel * ledsPerNote * (ch - 1));
    _LED.setPixelColor(ledIndex, 0, 0, 0);
  }
  count = 0;
}
